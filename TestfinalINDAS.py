# Elite Financial Analytics Platform - Complete Integration
# PhD-Level Penman-Nissim Analysis with Advanced Features
...
class PenmanNissimAnalyzer:
    ...
    def __init__(self, df: pd.DataFrame, mappings: Dict[str, Any]):
        ...
        # Validate and apply proxies
        self._validate_and_apply_proxies()

    def _validate_and_apply_proxies(self):
        """Validate required mappings and apply proxies if possible"""
        required = ['Total Assets', 'Total Liabilities', 'Total Equity', 'Revenue',
                   'Operating Income', 'Net Income', 'Net Financial Expense']
        required_multi = ['Financial Assets', 'Financial Liabilities']
        
        # Check singles
        for metric in required:
            if not self.mappings.get(metric):
                proxy = self._find_proxy(metric)
                if proxy:
                    self.mappings[metric] = proxy
                    self.proxies_used.append(f"Used proxy '{proxy}' for '{metric}'")
                else:
                    self.missing_metrics.append(metric)
        
        # Check multis
        for metric in required_multi:
            if not self.mappings.get(metric):
                self.proxies_used.append(f"No {metric} specified - assuming zero")
                self.mappings[metric] = []
        
        # Optional for quality
        optional = ['Operating Cash Flow', 'Accounts Receivable', 'Inventory',
                   'Accounts Payable', 'Gross Profit', 'Current Assets',
                   'Current Liabilities', 'Cash and Cash Equivalents']
        for metric in optional:
            if not self.mappings.get(metric):
                self.missing_metrics.append(f"(Optional) {metric}")

    def _find_proxy(self, metric: str) -> Optional[str]:
        """Find or compute a proxy for missing metric"""
        available = self.df.index.tolist()
        
        if metric == 'Operating Income':
            # Proxy: Gross Profit - Operating Expenses
            gp = next((m for m in available if 'gross profit' in m.lower()), None)
            oe = next((m for m in available if 'operating expense' in m.lower()), None)
            if gp and oe:
                return 'PROXY:GP-OE'  # Special flag for computed proxy
        
        elif metric == 'Net Financial Expense':
            # Proxy: Interest Expense
            ie = next((m for m in available if 'interest expense' in m.lower()), None)
            if ie:
                return ie
        
        elif metric == 'Revenue':
            # Proxy: Sales or Turnover
            return next((m for m in available if 'sales' in m.lower() or 'turnover' in m.lower()), None)
...
class DashboardApp:
    ...
    def _render_financial_analysis_tab(self, df: pd.DataFrame):
        ...
        if all_ratios_list:
            ...
            with col3:
                show_average = st.checkbox("Show Average Line", value=True, key="analysis_show_average")
